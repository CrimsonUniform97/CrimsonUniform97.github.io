...
public class WarGames extends MouseAdapter implements Runnable, KeyListener
{
    ...
    private void eraseTrail(Graphics g, int i)
    {
    ...
    private void checkLevel(Graphics g)
    {
        ...
        if(!alive)
        {
            ...
            g.setColor(Color.white);*/
            centerText(g, "All your cities are destroyed!", 240);
    ...
    private void centerText(Graphics g, String text, int x)
    {   
        g.setColor(Color.black);
        g.fillRect(x, 190, 200, 20);
        g.setColor(Color.white);
            ...
            for(int i=0; i<baseAlive.length && !running; i++)
            {
                if(baseAlive[i]) 
                {
                    ...
                    if(level > 0)
                    {
                        ...
                        centerText(g, "Level Complete!", 260);
                        centerText(g, "Bonus "+basesLeft+" x 1000!", 260);
                        
                        addScore(basesLeft*1000);
                        
                        centerText(g, "Level "+(level+1), 260);
                    }
